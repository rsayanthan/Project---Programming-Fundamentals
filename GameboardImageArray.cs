...
namespace GameboardGUI
{
    public class GameboardImageArray : UserControl
    {
        ...
        public GameboardImageArray(Form parentForm, int[,] gameBoardStateArray, Point topCorner, 
            ...
            int boardWidth = parentForm.ClientSize.Width - ((topCorner.X + 300) + (tileMargin * _boardRows - 1));
            _topY = topCorner.Y;
            _topX= topCorner.X;
            this._tileMargin = tileMargin;
            _tileImagesPath = tileImagePath;

            // bottomCorner.X - used in boardwidth above - instead of 500 to allow the game to fit alongside the panel
